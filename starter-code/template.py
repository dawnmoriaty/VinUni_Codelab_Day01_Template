...
def call_openai(
    ...
    return (
        ...
        latency,
        {
            "input_tokens": client_response.usage.prompt_tokens,
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Giữ lại tối đa 3 lượt hội thoại gần nhất (tối đa 6 tin nhắn)
        recent_history = history[-6:]
        messages = recent_history + [{"role": "user", "parts": [{"text": user_input}]}]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for item in results:
        ...
        for key, name in model_mapping:
            if key in item:
                stats = item[key]
                raw_resp = stats.get("response", "").replace("\n", " ")
                resp_trunc = (raw_resp[:47] + "...") if len(raw_resp) > 50 else raw_resp
                ...
                lines.append(f"| {prompt} | {name} | {resp_trunc} | {latency} | {tokens} | {cost} |")
    return "\n".join(lines)
